...
export const fetchTrendingMovies = async (): Promise<Movie[]> => {
  const res = await fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${API_KEY}&language=en-US&page=1`);
  if (!res.ok) throw new Error(`Error loading trending movies: ${res.status}`);
  const data = await res.json();
  return data.results as Movie[];
};
...
export const fetchMoviesByQuery = async (query: string): Promise<Movie[]> => {
  if (!query) return [];
  const res = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&page=1&query=${encodeURIComponent(query)}`);
  if (!res.ok) throw new Error(`Error searching movies: ${res.status}`);
  const data = await res.json();
  return data.results as Movie[];
};
